...
const Loader: FC<{ placeholderSrc?: string }> = ({ placeholderSrc }) => {
	...
	return (
		<Html center>
			{placeholderSrc ? <img src={placeholderSrc} width={128} height={128} className="blur-lg rounded-lg" /> : `${Math.round(progress)} %`}
		</Html>
	);
};

// Safe Environment wrapper that handles loading errors gracefully
const SafeEnvironment: FC<{ preset: string }> = ({ preset }) => {
	...
	useEffect(() => {
		...
		// Global error handler to catch HDR loading failures
		const handleError = (e: ErrorEvent) => {
			const errorMsg = e.message || e.error?.message || "";
			// Check for HDR loading errors or fetch failures related to environment maps
			if (
				errorMsg.includes(".hdr") ||
				errorMsg.includes("Failed to fetch") ||
				errorMsg.includes("studio") ||
		...
		// Also catch unhandled promise rejections (React Three Fiber uses promises)
	...
	if (hasError || preset === "none") return null;
	...
	useEffect(() => {
		...
		const move = (e: PointerEvent) => {
			...
			if (mode === "rotate") {
				...
				(camera as THREE.PerspectiveCamera).position.z = THREE.MathUtils.clamp(startZ * ratio, minZoom, maxZoom);
				invalidate();
			}
		};
